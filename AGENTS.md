...
- Before making code changes, read `DECISIONS.md` and follow all `accepted` decisions.
- If a requested change conflicts with an `accepted` decision, either:
  - pause and ask the user to confirm the override.
  - update `DECISIONS.md` with a new entry that supersedes it
- DO NOT silently violate recorded decisions.
