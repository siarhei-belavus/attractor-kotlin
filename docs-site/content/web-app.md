...
The System Tools grid shows detected binaries on the host. **Required** tools (`java`, `git`, `dot`) must be present for core features to work — a warning banner appears at the top of every page if any are missing. **Optional** tools are used by LLM-generated project stages and do not block Attractor itself.
